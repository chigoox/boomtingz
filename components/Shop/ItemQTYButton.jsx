...
const ItemQTYButton = ({ size = 'md', product, state, setState, }) => {
    ...
    const controlQTY = (action = 'add', count = 1) => {
        ...
        if (product) {
            const currentItemInfo = { images: product.images, name: product.name, price: product.metadata.price, priceID: product.priceID }
    ...
    return (
        <HStack style={tw``}>
            <Button onPress={() => controlQTY('sub')} style={tw`w-6 ${size == 'sm' ? 'p-4' : ''} scale-[2] bg-green-700 rounded-none rounded-l-full`}>
                <Center>
